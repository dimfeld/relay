...
- Publish Mail-owned `package.detected` events to Relay's internal API when package data needs delivery to OmniApp. Use a Mail service identity with only the required publish capability and a stable source event or idempotency key.
...
- If OmniApp publishes structured events to Relay, use its own service identity and the capabilities required by those event types.
